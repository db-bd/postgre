### Installtion from Zip File in Ubuntu
* Let the Unzipped Location be `` /opt/pgsql ``
* Add PostgreSQL `` bin `` folder location to `` ~/.bashrc `` File
* Create a `` data `` directory to postgresql folder
* Navigate to `` /opt/pgsql/bin `` directory
* Open `` terminal `` and run following command
```sh
initdb -U postgres -A password -E utf8 -W -D /opt/pgsql/data
```
...
```
* Now add following line ins ` ~/.bashrc ` File
```sh
alias postgre='pg_ctl -D /opt/pgsql/data -l logfile start'
```
* Start ` postgre ` server by following command
```sh
postgre
```
